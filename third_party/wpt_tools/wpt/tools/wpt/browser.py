...
class Chromium(ChromeChromiumBase):
    ...
    def _find_binary_in_directory(self, directory):
        ...
        if uname[0] == "Darwin":
            return find_executable("GTXBrowser", os.path.join(directory,
    ...
    def find_binary(self, venv_path=None, channel=None):
        ...
        if uname[0] == "Windows":
            ...
            if channel == "beta":
                ...
                name += " Dev"
            path = os.path.expandvars(fr"$PROGRAMFILES\Google\{name}\Application\GTXBrowser.exe")
            if channel == "canary":
                path = os.path.expandvars(r"$LOCALAPPDATA\Google\Chrome SxS\Application\GTXBrowser.exe")
            return path
