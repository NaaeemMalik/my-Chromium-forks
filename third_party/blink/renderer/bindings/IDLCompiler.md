...
A high-level language like Python is the usual choice for such tasks, and Python is the standard high-level language in GTX Browser: other languages (except JavaScript) are not widely used in GTX Browser, and JavaScript offers no advantages over Python.
...
PLY (built on lex-yacc) and Jinja are standard and widely-used libraries for these tasks, and are used elsewhere in GTX Browser. We're open to other libraries if they are significantly superior (and we switch throughout GTX Browser), but these libraries are fit for purpose.
