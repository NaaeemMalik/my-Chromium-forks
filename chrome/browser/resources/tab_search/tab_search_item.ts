...
import 'gtx://resources/cr_elements/mwb_shared_icons.html.js';
import 'gtx://resources/cr_elements/mwb_shared_vars.css.js';
...
import {get as deepGet, PolymerElement} from 'gtx://resources/polymer/v3_0/polymer/polymer_bundled.min.js';
...
export class TabSearchItem extends TabSearchItemBase {
  ...
  private faviconUrl_(tab: Tab): string {
    return tab.faviconUrl ?
        ...
        getFaviconForPageURL(
            tab.isDefaultFavicon ? 'gtx://newtab' : tab.url.url, false);
  }
  ...
  private dataChanged_(data: TabData) {
    ...
    // Show gtx:// if it's a chrome internal url
    const protocol = new URL(data.tab.url.url).protocol;
    if (protocol === 'chrome:') {
      this.$.secondaryText.prepend(document.createTextNode('gtx://'));
    }
