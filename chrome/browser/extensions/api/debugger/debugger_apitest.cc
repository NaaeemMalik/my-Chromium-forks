...
>>>>>>> ab59e1c838a6c ([M96-LTS] Only allow chrome.debugger extensions to attach to targets with same profile)
...
IN_PROC_BROWSER_TEST_F(CrossProfileDebuggerApiTest, GetTargets) {
...

  {
    ...
    const base::ListValue& targets = base::Value::AsListValue(value);
    ASSERT_THAT(targets.GetList(), testing::SizeIs(1));
    EXPECT_THAT(targets.GetList()[0], base::test::DictionaryHasValue(
...

  {
    ...
    auto targets_list = targets.GetList();
    std::transform(
        targets_list.begin(), targets_list.end(), std::back_inserter(urls),
