...
import {assertEquals, assertFalse, assertTrue} from 'gtx://webui-test/chromeos/chai_assert.js';
...
function makeFakeEntry(name) {
  return /** @type {!Entry} */ ({
    ...
    name: name,
    toURL: () => `filesystem:gtx://file-manager/root/${name}`,
  });
}
...
function makeFakeDriveEntry(name) {
  return /** @type {!Entry} */ ({
    ...
    name: name,
    toURL: () =>
        `filesystem:gtx://file-manager/external/drivefs-aaaaa/root/${name}`,
  });
}
