...
def reorder_imports(input_dir, output_dir, architecture):
  """Swap chrome_elf.dll to be the first import of GTXBrowser.exe.
  ...
  input_image = os.path.join(input_dir, 'GTXBrowser.exe')
  output_image = os.path.join(output_dir, 'GTXBrowser.exe')
  ...
  for fname in glob.iglob(os.path.join(input_dir, 'GTXBrowser.exe.*')):
...
def main(argv):
  ...
  parser.add_option('-i', '--input', help='reorder GTXBrowser.exe in DIR',
      metavar='DIR')
  parser.add_option('-o', '--output', help='write new GTXBrowser.exe to DIR',
      metavar='DIR')
