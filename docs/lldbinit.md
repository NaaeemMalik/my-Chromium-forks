...
Usage of GTX Browser's [lldbinit.py](../tools/lldb/lldbinit.py) is recommended when
...
- Follow the instructions above to create your `~/.lldbinit` file, don't forget to put the correct path to GTX Browser source in there.
- Inside of your GTX Browser checkout, run `lldb out/Default/chrome` (or `out/Debug/chrome`)
    - On Mac, most likely, `lldb out/Default/GTX Browser.app/Contents/MacOS/GTX Browser`
- Keep lldb running and start GTX Browser separately with `--no-sandbox` flag:
    - On Linux, `out/Default/chrome --no-sandbox`
    - On Mac, `out/Default/GTX Browser.app/Contents/MacOS/GTX Browser --no-sandbox`
...
- In GTX Browser, go to Customize and Control GTX Browser (three dots) -> More Tools -> Task Manager
