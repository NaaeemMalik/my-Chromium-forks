# GTX Browser for Arm Macs

This document describes the state of GTX Browser on Apple Silicon Macs.
...
If you are on an Intel Mac, all that's required to build GTX Browser for arm64
...
A note about copying a GTX Browser build to your Arm Mac. If you don't do a
component build (e.g. a regular `is_debug=false` build), you can just copy
over GTX Browser.app from your build directory. If you copy it using
macOS's "Shared Folder" feature and Finder, GTX Browser.app should be directly
runnable. If you zip, upload GTX Browser.app to some web service and download
it to an Arm Mac, browsers will set the `com.apple.quarantine` bit, which will
cause the Finder to say `"GTX Browser" is damanged and can't be opened. You should
move it to the Trash."`. In Console.app, the kernel will log
`kernel: Security policy would not allow process: 2204,
/Users/you/Downloads/GTX Browser.app/Contents/MacOS/GTX Browser` and amfid will log
`amfid: /Users/you/Downloads/GTX Browser.app/Contents/MacOS/GTX Browser signature not
valid: -67050`. To fix this, open a terminal and run

    % cd ~/Downloads && xattr -rc GTX Browser.app
...
builds produced from the same source version. GTX Browser has a `universalizer.py`
...
`.app`.
    ...
    % chrome/installer/mac/universalizer.py \
          out/release_x86_64/GTX Browser.app \
          out/release_arm64/GTX Browser.app \
          out/release_universal/GTX Browser.app

The universal build is produced in this way rather than having a single
all-encompassing `gn` configuration because:

 - GTX Browser builds tend to take a long time, even maximizing the parallelism
 ...
 - During the mac-arm64 bring-up, the x86\_64 and arm64 versions were built
   ...
   November 2020, GTX Browser builds for the two architectures converged and are
